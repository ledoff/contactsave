    ...
    private void dbsynchro(SQLiteDatabase db, String res) {
       // Cursor c = db.query(tbl, null, null, null, null, null, null);
        ...
        try {
            ...
            JSONArray datamesto = dataJsonObj.getJSONArray("mesto");
            db.delete("mytablebase", null, null); // сначала удаляем все на всякий случай чтобы не двоить
            for(int i=0;i<dataname.length();i++ ){
                ...
                cv.put("kol", 1);
                cv.put("name2", datamesto.getString(i).trim());
                // переход на следующую строку
                // а если следующей нет (текущая - последняя), то false - выходим из цикла
                nint = nint + 1;
                db.insert("mytablebase", null, cv);   // и записываем
    ...
    class Task3 extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... urls) {
            ...
            try {
                ...
                nameValuePairs.add(new BasicNameValuePair("filename", URLEncoder.encode(dn.getDeviceName()+"_"+cquery, "UTF-8")));

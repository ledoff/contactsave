...
//we have a result
    ...
    private void findbase(String strkod) {
        ...
        Cursor c;
        // сначала смотрим пустая ли база, если пустая то никакого поиска не делаем
        String sqlQuery = "select name,inum,name2 "
                + "from mytablebase  "
                + " ";
        c = db.rawQuery(sqlQuery,null);
        if (c.getCount()!=0) {
            sqlQuery = "select name,inum,name2 "
                       + "from mytablebase  "
                       + "where kod='" + strkod + "'";
            c = db.rawQuery(sqlQuery, null);
            if (c.getCount() != 0) {
                c.moveToFirst();
                textView.setText(c.getString(c.getColumnIndex("inum")) + " " + c.getString(c.getColumnIndex("name"))+" \n "+c.getString(c.getColumnIndex("name2")));
            ...
            entvvod(strkod); // пустая просто выводим код
        }
        c.close();
        dbHelper.close();
